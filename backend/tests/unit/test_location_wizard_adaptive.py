...
class LocationStructureTests(unittest.TestCase):
    ...
    def test_grid_orientation_changes_physical_coordinates_only(self):
        ...
        locations = cellar_rules.generate_locations(scheme)
        by_code = {item["import"]: item for item in locations}
        self.assertEqual((by_code["MC3"]["row"], by_code["MC3"]["column"]), (0, 0))
        self.assertEqual((by_code["MA1"]["row"], by_code["MA1"]["column"]), (2, 2))

    def test_depth_bottom_to_top_places_row_one_at_bottom(self):
        scheme = {
            "kind": "depth",
            "prefix": "G",
            "row_start": 1,
        ...
        locations = cellar_rules.generate_locations(scheme)
        by_code = {item["import"]: item for item in locations}
        self.assertEqual((by_code["G5F"]["row"], by_code["G5F"]["column"]), (0, 0))
        self.assertEqual((by_code["G1B"]["row"], by_code["G1B"]["column"]), (4, 1))
