...
class TestWineRepo(DatabaseTestCase):
    ...
    def test_identity_lookup_distinguishes_vintages(self):
        w2020 = Wine(id=new_id(), producer="X", cuvee="Y", appellation="Z", vintage=2020, color="red", format="75cl")
        w2021 = Wine(id=new_id(), producer="X", cuvee="Y", appellation="Z", vintage=2021, color="red", format="75cl")
...
class TestCellarRepo(DatabaseTestCase):
    def test_name_uniqueness_case_insensitive(self):
        repo.insert_cellar(self.conn, Cellar(id=new_id(), name="Cave Nord", max_capacity=10, threshold=8))
        with self.assertRaises(ConflictError):
            repo.insert_cellar(self.conn, Cellar(id=new_id(), name="cave nord", max_capacity=10, threshold=8))
    ...
    def test_delete_blocked_while_holdings_reference_it(self):
        ...
        w = Wine(id=new_id(), producer="X", color="red")
        repo.insert_wine(self.conn, w)
        repo.insert_holding(self.conn, Holding(id=new_id(), wine_id=w.id, cellar_id=c.id, quantity=1))
        with self.assertRaises(ConflictError):
    ...
    def test_cellar_fill_sums_active_holdings_only(self):
        ...
        w = Wine(id=new_id(), producer="X", color="red")
        repo.insert_wine(self.conn, w)
        repo.insert_holding(self.conn, Holding(id=new_id(), wine_id=w.id, cellar_id=c.id, quantity=4, state="in_cellar"))
        repo.insert_holding(self.conn, Holding(id=new_id(), wine_id=w.id, cellar_id=c.id, quantity=2, state="drunk"))
...
class TestMovementRepo(DatabaseTestCase):
    def test_client_op_id_dedup(self):
        ...
        self.assertIsNone(repo.insert_movement(self.conn, m2), "second insert with same client_op_id must be a no-op")

    def test_list_movements_filters_by_cellar(self):
        m1 = Movement(id=new_id(), action="move", from_cellar_id="c1", to_cellar_id="c2", quantity_delta=1)
        m2 = Movement(id=new_id(), action="move", from_cellar_id="c3", to_cellar_id="c4", quantity_delta=1)
...
class TestUserRepo(DatabaseTestCase):
    def test_username_uniqueness_case_insensitive(self):
        repo.insert_user(self.conn, User(id=new_id(), username="Alice", password_hash="h", password_salt="s"))
        with self.assertRaises(ConflictError):
            repo.insert_user(self.conn, User(id=new_id(), username="alice", password_hash="h2", password_salt="s2"))
