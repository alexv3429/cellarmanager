...
def dw(after=None, before=None, confidence=0.5, source="src"):
    return enrichment.DrinkingWindowResult(
        drink_after=after, drink_before=before, confidence=confidence, source=source
    )


def mi(value=None, confidence=0.5, source="src", pairing=None, experience=None):
    return enrichment.MarketInfoResult(
        market_value=value,
        advice_pairing=pairing,
        advice_experience=experience,
...
class TestAggregateDrinkingWindows(unittest.TestCase):
    ...
    def test_single_source_passthrough(self):
        result = enrichment.aggregate_drinking_windows(
    ...
    def test_agreeing_sources_increase_confidence_above_the_average(self):
        ...
        self.assertGreater(
            result.confidence,
            0.4,
            "three sources agreeing closely should be more trustworthy than any one alone",
        )
    ...
    def test_wildly_disagreeing_sources_decrease_confidence_below_the_average(self):
        ...
        self.assertLess(
            result.confidence,
            0.5,
            "wildly disagreeing sources should erode confidence even if individually plausible",
        )
    ...
    def test_none_entries_in_list_are_ignored(self):
        results = [
            None,
            dw(after=date(2023, 1, 1), before=date(2030, 1, 1), confidence=0.5, source="a"),
            None,
        ]
        result = enrichment.aggregate_drinking_windows(results)
        self.assertEqual(result.source_count, 1)
...
class TestAggregateMarketInfo(unittest.TestCase):
    ...
    def test_agreeing_prices_increase_confidence(self):
        results = [
            mi(value=20.0, confidence=0.4, source="a"),
            mi(value=21.0, confidence=0.4, source="b"),
            mi(value=19.5, confidence=0.4, source="c"),
        ]
    ...
    def test_wildly_different_prices_decrease_confidence(self):
        results = [
            mi(value=10.0, confidence=0.5, source="a"),
            mi(value=500.0, confidence=0.5, source="b"),
        ]
        result = enrichment.aggregate_market_info(results)
...
class TestMergeValue(unittest.TestCase):
    def test_fills_missing_value(self):
        decision = enrichment.merge_value(
            existing_value=None,
            existing_confidence=None,
            fetched_value=42,
            fetched_confidence=0.5,
            fetched_source="test",
        )
...
class TestApplyDrinkingWindowEnrichment(unittest.TestCase):
    def test_applies_aggregated_result_to_empty_wine(self):
        wine = _wine()
        aggregated = enrichment.aggregate_drinking_windows(
            [dw(after=date(2025, 1, 1), before=date(2030, 1, 1), confidence=0.4, source="mock-a")]
        )
    ...
    def test_does_not_overwrite_manual_dates(self):
        wine = _wine(
            drink_after=date(2024, 1, 1), drink_after_confidence=1.0, drink_after_source="manual"
        )
        aggregated = enrichment.aggregate_drinking_windows(
            [dw(after=date(2099, 1, 1), confidence=0.9, source="mock-a")]
        )
        enrichment.apply_drinking_window_enrichment(wine, aggregated)
        self.assertEqual(
            wine.drink_after,
            date(2024, 1, 1),
            "manual value must survive a fetched (even high-confidence) aggregate",
        )


class TestMultipleProvidersRegistered(unittest.TestCase):
    def setUp(self):
        self._previous_demo_enrichment = os.environ.get("WINECELLAR_ENABLE_DEMO_ENRICHMENT")
    ...
    def tearDown(self):
        if self._previous_demo_enrichment is None:
            ...
            os.environ["WINECELLAR_ENABLE_DEMO_ENRICHMENT"] = self._previous_demo_enrichment

    def test_get_active_providers_returns_more_than_one(self):
        providers = enrichment.get_active_providers()
        self.assertGreaterEqual(
            len(providers), 2, "the whole point is combining several sources, not just one"
        )

    def test_fetch_and_aggregate_combines_all_registered_providers(self):
        wine = _wine(vintage=2018)
        result = enrichment.fetch_and_aggregate_drinking_window(
            wine, enrichment.get_active_providers()
        )
        self.assertIsNotNone(result)
        self.assertEqual(result.source_count, len(enrichment.get_active_providers()))
        self.assertLess(
            result.confidence,
            enrichment.MANUAL_CONFIDENCE,
            "aggregated/fetched data must never claim manual-level confidence",
        )
