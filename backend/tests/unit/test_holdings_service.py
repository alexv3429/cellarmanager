...
class TestAddBottles(DatabaseTestCase):
    ...
    def test_add_creates_holding_and_journal_entry(self):
        result = hs.add_bottles(self.conn, wine_id=self.wine.id, cellar_id=self.cellar.id, location="A1", quantity=6)
    ...
    def test_add_merges_into_existing_holding(self):
        hs.add_bottles(self.conn, wine_id=self.wine.id, cellar_id=self.cellar.id, location="A1", quantity=3)
        result = hs.add_bottles(self.conn, wine_id=self.wine.id, cellar_id=self.cellar.id, location="A1", quantity=2)
    ...
    def test_add_warns_over_threshold(self):
        result = hs.add_bottles(self.conn, wine_id=self.wine.id, cellar_id=self.cellar.id, location="A1", quantity=9)
        self.assertIsNotNone(result.warning)

    def test_add_zero_or_negative_rejected(self):
        with self.assertRaises(ValidationError):
            hs.add_bottles(self.conn, wine_id=self.wine.id, cellar_id=self.cellar.id, location="A1", quantity=0)
...
class TestMoveBottles(DatabaseTestCase):
    def setUp(self):
        ...
        self.add_result = hs.add_bottles(self.conn, wine_id=self.wine.id, cellar_id=self.cellar_a.id, location="A1", quantity=10)

    def test_full_move(self):
        result = hs.move_bottles(
            self.conn, holding_id=self.add_result.holding.id, quantity=10,
            to_cellar_id=self.cellar_b.id, to_location="B1",
        )
    ...
    def test_partial_move_splits_holding(self):
        hs.move_bottles(self.conn, holding_id=self.add_result.holding.id, quantity=4, to_cellar_id=self.cellar_b.id, to_location="B1")
    ...
    def test_cannot_move_more_than_available(self):
        with self.assertRaises(ValidationError):
            hs.move_bottles(self.conn, holding_id=self.add_result.holding.id, quantity=999, to_cellar_id=self.cellar_b.id, to_location="B1")

    def test_move_merges_into_existing_destination_holding(self):
        hs.add_bottles(self.conn, wine_id=self.wine.id, cellar_id=self.cellar_b.id, location="B1", quantity=5)
        hs.move_bottles(self.conn, holding_id=self.add_result.holding.id, quantity=3, to_cellar_id=self.cellar_b.id, to_location="B1")
        holdings_b = repo.list_holdings(self.conn, cellar_id=self.cellar_b.id, active_only=True)
        self.assertEqual(len(holdings_b), 1, "moving into a spot with an existing holding should merge, not fragment")
...
class TestRemoveBottles(DatabaseTestCase):
    def setUp(self):
        ...
        repo.insert_cellar(self.conn, self.cellar)
        self.add_result = hs.add_bottles(self.conn, wine_id=self.wine.id, cellar_id=self.cellar.id, location="A1", quantity=10)

    def test_partial_removal_splits_and_tags_state(self):
        result = hs.remove_bottles(
            self.conn, holding_id=self.add_result.holding.id, quantity=2,
            reason=HoldingState.GIFTED, note="birthday gift",
        )
    ...
    def test_cellar_fill_drops_after_removal(self):
        hs.remove_bottles(self.conn, holding_id=self.add_result.holding.id, quantity=3, reason=HoldingState.DRUNK)
        self.assertEqual(repo.cellar_fill(self.conn, self.cellar.id), 7)

    def test_invalid_reason_rejected(self):
        with self.assertRaises(ValidationError):
            hs.remove_bottles(self.conn, holding_id=self.add_result.holding.id, quantity=1, reason=HoldingState.IN_CELLAR)

    def test_journal_records_negative_quantity_delta(self):
        result = hs.remove_bottles(self.conn, holding_id=self.add_result.holding.id, quantity=3, reason=HoldingState.LOST)
