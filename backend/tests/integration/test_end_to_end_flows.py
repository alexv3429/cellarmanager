...
from app.services import csv_io, moveplan_service, stats_service
...
class TestFullCellarLifecycle(DatabaseTestCase):
    ...
    def _import_sample(self):
        csv_bytes = (
            b"Producer,Cuvee,Appellation,Vintage,Color,Area,Format,Quantity,Price bought,"
            b"Cellar,Location,Drink after,Drink before,Advice for dish association\n"
            b"Domaine Jean-Marc Burgaud,James,Cote du Py,2020,red,Beaujolais,75cl,12,18.50,"
            b"Cave Nord,AG1,2023-01-01,2028-01-01,grilled meat and charcuterie\n"
            b"Veuve Cliquot,Brut,Champagne,,sparkling,Champagne,75cl,6,32,"
            b"Kitchen Fridge,SV1,,,seafood and celebrations\n"
            b"Chateau Margaux,Grand Vin,Margaux,2018,red,Bordeaux,75cl,3,450,"
            b"Cave Nord,AG2,2030-01-01,2045-01-01,slow-roasted lamb\n"
        )
    ...
    def test_full_lifecycle(self):
        ...
        next(w for w in wines if w.producer == "Veuve Cliquot")

        # journal has one IMPORT movement per row
        self.assertEqual(len(repo.list_movements(self.conn)), 3)

        # --- 3. Add / move / remove actions -------------------------------
        repo.list_holdings(self.conn, wine_id=burgaud.id)[0]
        add_result = hs.add_bottles(
            self.conn, wine_id=burgaud.id, cellar_id=self.aging.id, location="AG1", quantity=6
        )
        ...
        remove_result = hs.remove_bottles(
            self.conn,
            holding_id=move_result.holding.id,
            quantity=2,
        ...
        expected_total = sum(
            h.quantity
            for h in repo.list_holdings(self.conn, active_only=True)
            if h.state == "in_cellar"
        )
        ...
        export_rows = [
            (w, h, repo.get_cellar(self.conn, h.cellar_id) if h.cellar_id else None)
            for h, w in repo.list_holdings_with_wines(self.conn)
        ]
        csv_text = csv_io.export_csv(
            export_rows,
            columns=["producer", "vintage", "color", "cellar", "quantity"],
            language="fr",
        )
    ...
    def test_reimport_is_idempotent_for_totals(self):
        self._import_sample()
        totals_after_first = sum(
            h.quantity for h in repo.list_holdings(self.conn, active_only=True)
        )
        self._import_sample()
        totals_after_second = sum(
            h.quantity for h in repo.list_holdings(self.conn, active_only=True)
        )
        self.assertEqual(
            totals_after_second,
            totals_after_first * 2,
            "re-importing the same file adds the same quantities again, without duplicating wines",
        )
        self.assertEqual(
            len(repo.list_wines(self.conn)), 3, "wine catalog must not duplicate on re-import"
        )
