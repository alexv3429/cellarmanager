...
-- Estate words such as "Chateau" or "Domaine" are useful display text but
-- unsafe identity keys: a short generic LWIN producer can otherwise receive a
-- perfect word-similarity score. Rank producer candidates on both the full
-- name and the remaining distinctive terms.
create or replace function private.wine_reference_producer_identity_terms(
    p_value text
)
returns text
language sql
...
as $$
    select coalesce(
        string_agg(token.value, ' ' order by token.ordinality),
        ''
    )
    from pg_catalog.regexp_split_to_table(
        private.normalize_wine_reference_text(p_value),
        '[[:space:]]+'
    ) with ordinality token(value, ordinality)
    where token.value <> all(array[
        'chateau',
        'clos',
        'domaine',
        'domain',
...
revoke execute
on function private.wine_reference_producer_identity_terms(text)
    ...
    with grouped as (
        ...
        where snapshot.source_key = 'liv-ex-lwin'
          ...
          and (
              ...
              or private.wine_reference_producer_identity_terms(
                  entry.producer_search
              ) like '%' || v_source_terms || '%'
              or v_source_terms like '%' ||
                  private.wine_reference_producer_identity_terms(
                      entry.producer_search
                  ) || '%'
          )
...
begin
    ...
    v_candidate_terms := private.wine_reference_producer_identity_terms(
        v_producer_key
    );
    ...
    select
        ...
        max(greatest(
            extensions.similarity(v_source_producer, entry.producer_search),
            extensions.similarity(v_source_terms, v_candidate_terms)
        ))
    ...
    if v_source_terms = ''
       or v_candidate_terms = ''
       or v_canonical_name is null
       or extensions.similarity(v_source_terms, v_candidate_terms) < 0.25
       or v_score < 0.25
    then
