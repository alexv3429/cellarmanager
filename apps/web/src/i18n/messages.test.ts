...
describe("French dynamic interface messages", () => {
  ...
  it("translates generated maturity, confidence, and sync copy", () => {
    ...
    expect(translate("fr", "Sparkling")).toBe("Effervescent")
    expect(translate("fr", "Fortified")).toBe("Vin de liqueur")
    expect(translate("fr", "fortified")).toBe("vin de liqueur")
    expect(translate("fr", "other")).toBe("autre")
    expect(translate("fr", "csv.export.includeZeroStockOne", { count: "1" }))
      .toBe("Inclure 1 vin sans bouteille")
    expect(translate("fr", "csv.export.includeZeroStockMany", { count: "776" }))
      .toBe("Inclure 776 vins sans bouteille")
  })
})
