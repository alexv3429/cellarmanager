...
`data.full_name` for the display name, preserving other metadata. The existing
...
3. As Member, open Account and confirm the same own-account controls. No control
   edits another collaborator's account, and shared-cellar permissions stay the same.
